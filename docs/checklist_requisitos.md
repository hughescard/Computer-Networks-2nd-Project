...
| --- | --------------------------- | ---------------------------- | --------------------- |
| R1  | Topologia y forwarding base: `docs/topologia.md`, `scripts/firewall_init.sh` habilita `ip_forward` y politicas DROP | En el gateway: `cat /proc/sys/net/ipv4/ip_forward` -> 1 y `sudo iptables -L FORWARD -n` muestra politica DROP por defecto | Pendiente lab |
| R2  | Redireccion HTTP al portal (issue 13): regla `REDIRECT` en `scripts/firewall_init.sh`, detalle en `docs/firewall.md` | Cliente sin login: `curl -i http://example.com` debe mostrar el portal; gateway: `sudo iptables -t nat -L PREROUTING -n --line-numbers | grep REDIRECT` | Pendiente lab |
...
| R4  | UI del portal (login/success/error/logout): plantillas en `src/templates/` con mensajes claros (issue 17) | GET `/login` y revisar que se muestren instrucciones y mensajes de error/resultado coherentes | Pendiente lab |
| R5  | Archivo de usuarios y autenticacion basica: `config/usuarios.txt`, loader en `src/auth.py` | `python3 src/auth.py` para cargar usuarios y validar credenciales; login con usuario valido/invalidos | Pendiente lab |
